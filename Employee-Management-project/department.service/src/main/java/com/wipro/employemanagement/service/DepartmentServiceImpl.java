...
public class DepartmentServiceImpl implements DepartmentService {
    ...
    public DepartmentServiceImpl(DepartmentRepository departmentRepository, EmployeeClient employeeClient, ProjectClient projectClient) {
...
List<EmployeeDTO> list = employeesByDepartment.getBody();
 ...
 System.out.println(list);
	    // 3. Get projects from project-service
	    List<ProjectDTO> projects = projectClient.getProjectsByEmployeeIds(employeeIds);
System.out.println(projects);
	    // 4. Remove duplicates by project ID
	    return projects.stream()
	            .collect(Collectors.toMap(ProjectDTO::getId, p -> p, (p1, p2) -> p1))
	            .values()
	            .stream()
	            .toList();
	}
